...
def reflectVector(normal, dirVector):
    # R = 2 * ( N . L) * N - L
    reflect = 2 * ml.dot(normal, dirVector)
    reflect = ml.multiply(reflect, normal)
    reflect = ml.subtract(reflect, dirVector)
    reflect = reflect / ml.linalg.norm(reflect)
    return reflect


def refractVector(normal, dirVector, ior):
    # Snell's Law
    cosi = max(-1, min(1 , ml.dot(dirVector, normal)))
    etai = 1
    etat = ior
    ...
    R = eta * ml.array(dirVector) + (eta * cosi - k**0.5) * normal
    return R / ml.linalg.norm(R)
...
class Raytracer(object):
    ...
    def glRender(self):
        ...
        for y in range(0, self.height, STEPS):
            for x in range(0, self.width , STEPS):
                ...
                direction = direction / ml.linalg.norm(direction)
    ...
    def cast_ray(self, orig, dir, origObj = None, recursion = 0):
        ...
        # Colors
        finalColor = ml.array([0,0,0])
        objectColor = ml.array([material.diffuse[0],
                                material.diffuse[1],
                                material.diffuse[2]])

        ambientColor = ml.array([0,0,0])
        dirLightColor = ml.array([0,0,0])
        pLightColor = ml.array([0,0,0])
        finalSpecColor = ml.array([0,0,0])
        reflectColor = ml.array([0,0,0])
        refractColor = ml.array([0,0,0])




        # Direccion de vista
        view_dir = ml.subtract(self.camPosition, intersect.point)
        view_dir = view_dir / ml.linalg.norm(view_dir)

        if self.ambLight:
            ambientColor = ml.array(self.ambLight.getColor())

        if self.dirLight:
            diffuseColor = ml.array([0,0,0])
            specColor = ml.array([0,0,0])
            shadow_intensity = 0

            # Iluminacion difusa
            light_dir = ml.array( self.dirLight.direction) * -1
            intensity = max(0, ml.dot(intersect.normal, light_dir)) * self.dirLight.intensity
            diffuseColor = ml.array([intensity * self.dirLight.color[0],
                                     intensity * self.dirLight.color[1],
                                     intensity * self.dirLight.color[2]])

            # Iluminacion especular
            reflect = reflectVector(intersect.normal, light_dir)
            spec_intensity = self.dirLight.intensity * max(0,ml.dot(view_dir, reflect)) ** material.spec
            specColor = ml.array([spec_intensity * self.dirLight.color[0],
            ...
            finalSpecColor = ml.add(finalSpecColor, (1 - shadow_intensity) * specColor)
        ...
        for pointLight in self.pointLights:
            ...
            # Iluminacion difusa
            light_dir = ml.subtract(pointLight.position, intersect.point)
            light_dir = light_dir / ml.linalg.norm(light_dir)
            intensity = max(0, ml.dot(intersect.normal, light_dir)) * pointLight.intensity
            diffuseColor = ml.array([intensity * pointLight.color[0],
                                     intensity * pointLight.color[1],
                                     intensity * pointLight.color[2]])

            # Iluminacion especular
            reflect = reflectVector(intersect.normal, light_dir)
            spec_intensity = pointLight.intensity * max(0,ml.dot(view_dir, reflect)) ** material.spec
            specColor = ml.array([spec_intensity * pointLight.color[0],
                                  spec_intensity * pointLight.color[1],
                                  spec_intensity * pointLight.color[2]])

            # Shadows
            shadInter = self.scene_intersect(intersect.point, light_dir, intersect.sceneObject)
            lightDistance = ml.linalg.norm( ml.subtract(pointLight.position, intersect.point) )
            if shadInter and shadInter.distance < lightDistance:
                shadow_intensity = 1

            pLightColor = ml.add(pLightColor, (1 - shadow_intensity) * diffuseColor)
            finalSpecColor = ml.add(finalSpecColor, (1 - shadow_intensity) * specColor)
        ...
        if material.matType == OPAQUE:
            ...
            if material.texture and intersect.texCoords:
                ...
                finalColor *= ml.array(texColor)

        elif material.matType == REFLECTIVE:
            reflect = reflectVector(intersect.normal, ml.array(dir) * -1)
            reflectColor = self.cast_ray(intersect.point, reflect, intersect.sceneObject, recursion + 1)
            reflectColor = ml.array(reflectColor)
            ...
            if kr < 1:
                ...
                refractColor = ml.array(refractColor)
